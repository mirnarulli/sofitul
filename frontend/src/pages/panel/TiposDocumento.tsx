    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-blue-200 p-5 mb-4">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">{editId ? 'Editar' : 'Nuevo tipo de documento'}</h2>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Código (ej: CI)</label>
              <input value={form.codigo} onChange={e => setForm(f => ({ ...f, codigo: e.target.value.toUpperCase() }))} className={inputCls} maxLength={10} />
            </div>
            <div>
            ...
            <tbody className="divide-y divide-gray-100">
              {tipos.map((t: any) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono font-bold text-gray-700">{t.codigo}</td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => { setEditId(t.id); setForm({ codigo: t.codigo, nombre: t.nombre, activo: t.activo }); setShowForm(true); }}
