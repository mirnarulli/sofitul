...
const TABS = ['Datos Empresa','Rep. Legal','Operaciones','Transferencia','Documentos','Due Diligencia','Estado de Cuenta'] as const;
...
export default function ContactoPJDetalle() {
  ...
  // Cargar detalle de operación para Estado de Cuenta
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([opFull, tx, cargos, medios]) => {
        setEcOp(opFull); setEcTx(tx); setEcCargos(cargos); setEcMedios(medios);
      })
      .catch(() => {})
      .finally(() => setEcLoading(false));
  }, [tab, ecOpId]);

  // Auto-seleccionar primera operación al entrar al tab
  useEffect(() => {
  ...
  const renderEstadoCuenta = () => {
    ...
    const getMedio = (mid?: string) => mid ? (ecMedios.find((m: any) => m.id === mid)?.nombre ?? '—') : '—';
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
          ...
          {ecOpId && (
            <a href={`/operaciones/${ecOpId}/estado-cuenta`} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 whitespace-nowrap font-medium">
              🖨 Imprimir
            </a>
          )}
        </div>

        {ecLoading && <div className="text-center py-10 text-gray-400 text-sm">Cargando...</div>}
